      ...
      {rows.map((row, index) => {
        ...
        return (
          <div key={index} className="flex items-center gap-1.5">
            ...
            >
              <option value="plain">{t("envVar.plain")}</option>
              <option value="secret">{t("envVar.secret")}</option>
            </select>
            {row.source === "secret" ? (
              <>
                <select
                ...
                >
                  <option value="">{t("envVar.selectSecret")}</option>
                  {row.secretId && !secrets.some((s) => s.id === row.secretId) ? (
                    <option value={row.secretId}>{t("envVar.missingSecret").replace("{id}", row.secretId.slice(0, 8))}</option>
                  ) : null}
                  {secrets.map((secret) => (
