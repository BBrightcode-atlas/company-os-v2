  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger className="flex w-full items-center justify-between rounded-lg border border-border/70 px-3 py-2 text-left">
        <div>
          <p className="text-sm font-medium">{t("routineVars.variables")}</p>
          <p className="text-xs text-muted-foreground">
            {t("routineVars.detectedHint")}
          </p>
        </div>
          ...
          <div key={variable.name} className="rounded-lg border border-border/70 p-4">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                {t("routineVars.promptHint")}
              </span>
            </div>

            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routineVars.label")}</Label>
                <Input
              ...
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routineVars.type")}</Label>
                <Select
                  value={variable.type}
              ...
              <div className="space-y-1.5 md:col-span-2">
                <div className="flex items-center justify-between gap-3">
                  <Label className="text-xs">{t("routineVars.defaultValue")}</Label>
                ...
                {variable.type === "textarea" ? (
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="__unset__">{t("routineVars.noDefault")}</SelectItem>
                      <SelectItem value="true">{t("routineVars.true")}</SelectItem>
                      <SelectItem value="false">{t("routineVars.false")}</SelectItem>
                    </SelectContent>
                  </Select>
                ) : variable.type === "select" ? (
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("routineVars.options")}</Label>
                      <Input
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("routineVars.defaultOption")}</Label>
                      <Select
                      ...
                      >
                        <SelectTrigger>
                          <SelectValue placeholder={t("routineVars.noDefault")} />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="__unset__">{t("routineVars.noDefault")}</SelectItem>
                  ...
                  <Input
                    ...
                    placeholder={variable.type === "number" ? "42" : t("routineVars.defaultValue")}
                  />
                )}
              </div>
            </div>
