...
const PROJECT_STATUS_KEYS: Record<string, string> = {
  backlog: "projectProps.backlog",
  planned: "projectProps.planned",
  in_progress: "projectProps.inProgress",
  completed: "projectProps.completed",
  cancelled: "projectProps.cancelled",
};
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {t((PROJECT_STATUS_KEYS[status] ?? "projectProps.backlog") as any)}
        </button>
      </PopoverTrigger>
      <PopoverContent className="w-40 p-1" align="start">
        {PROJECT_STATUS_VALUES.map((value) => (
          ...
          >
            {t((PROJECT_STATUS_KEYS[value] ?? "projectProps.backlog") as any)}
          </Button>
        ))}
      </PopoverContent>
    </Popover>
  ...
  return (
    <div className="space-y-3 rounded-md border border-destructive/40 bg-destructive/5 px-4 py-4">
      <p className="text-sm text-muted-foreground">
        {isArchive
          ? t("projectProps.archiveDesc")
          : t("projectProps.unarchiveDesc")}
      </p>
      {archivePending ? (
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-destructive font-medium">
            {isArchive ? t("projectProps.archive") : t("projectProps.unarchive")} &ldquo;{project.name}&rdquo;?
          </span>
          <Button
            size="sm"
            variant="destructive"
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const submitLocalWorkspace = () => {
    ...
    if (!isAbsolutePath(cwd)) {
      setWorkspaceError(t("projectProps.localFolderAbsPath"));
      return;
    }
    setWorkspaceError(null);
    persistCodebase({ cwd });
  ...
  const submitRepoWorkspace = () => {
    ...
    if (!looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(t("projectProps.repoInvalidUrl"));
      return;
    }
    setWorkspaceError(null);
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-lg bg-card p-4 space-y-1">
        <PropertyRow label={<FieldLabel label={t("projectProps.name")} state={fieldState("name")} />}>
          {onUpdate || onFieldUpdate ? (
            <DraftInput
              ...
              placeholder={t("projectProps.projectName")}
            />
          ) : (
        ...
        >
          {onUpdate || onFieldUpdate ? (
            <InlineEditor
              ...
              placeholder={t("projectProps.addDescription")}
              multiline
            />
          ) : (
            <p className="text-sm text-muted-foreground">
        ...
        <PropertyRow label={<FieldLabel label={t("projectProps.status")} state={fieldState("status")} />}>
          {onUpdate || onFieldUpdate ? (
            <ProjectStatusPicker
        ...
        {project.targetDate && (
          <PropertyRow label={<FieldLabel label={t("projectProps.targetDate")} state="idle" />}>
      ...
      <div className="rounded-lg bg-card p-4 space-y-1">
        <div className="space-y-2">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <Tooltip>
              ...
              <TooltipContent side="top">
                {t("projectProps.codebaseTooltip")}
              </TooltipContent>
            </Tooltip>
          </div>
          ...
          {createWorkspace.isError && (
            <p className="text-xs text-destructive">{t("projectProps.failedSaveWorkspace")}</p>
          )}
          {removeWorkspace.isError && (
            <p className="text-xs text-destructive">{t("projectProps.failedDeleteWorkspace")}</p>
          )}
          {updateWorkspace.isError && (
            <p className="text-xs text-destructive">{t("projectProps.failedUpdateWorkspace")}</p>
          )}
        </div>
            ...
            <div className="mt-4 pt-4 border-t border-border/30 space-y-2">
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                ...
                <Tooltip>
                  ...
                  <TooltipContent side="top">
                    {t("projectProps.executionWorkspacesTooltip")}
                  </TooltipContent>
                </Tooltip>
              </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2 text-sm font-medium">
                      <span>{t("projectProps.enableIsolatedCheckouts")}</span>
                      <SaveIndicator state={fieldState("execution_workspace_enabled")} />
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {t("projectProps.enableIsolatedCheckoutsDesc")}
                    </div>
                  </div>
                  {onUpdate || onFieldUpdate ? (
                    <ToggleSwitch
                ...
                {executionWorkspacesEnabled ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2 text-sm">
                          <span>{t("projectProps.newIssuesDefaultIsolated")}</span>
                          <SaveIndicator state={fieldState("execution_workspace_default_mode")} />
                        </div>
                        <div className="text-[11px] text-muted-foreground">
                          {t("projectProps.newIssuesDefaultIsolatedDesc")}
                        </div>
                      </div>
                      <ToggleSwitch
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        <div className="text-xs text-muted-foreground">
                          {t("projectProps.hostImplementation")} <span className="text-foreground">{t("projectProps.gitWorktree")}</span>
                        </div>
                        <div>
