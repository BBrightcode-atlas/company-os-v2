...
export function BudgetPolicyCard({
    ...
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? t("budgetPolicy.ofLimit").replace("{percent}", String(summary.utilizationPercent)) : t("budgetPolicy.noCapConfigured")}
        </div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("budgetPolicy.budget")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("projectProps.disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {t("budgetPolicy.softAlert").replace("{percent}", String(summary.warnPercent))}{summary.paused && summary.pauseReason ? ` · ${summary.pauseReason} pause` : ""}
        </div>
      </div>
    </div>
  ) : (
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? t("misc.saving") : summary.amount > 0 ? t("budgetPolicy.updateBudget") : t("budgetPolicy.setBudget")}
      </Button>
    </div>
  ) : null;
  ...
  if (isPlain) {
    return (
      <div className="space-y-6">
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("projectProps.invalidDollarAmount")}</p>
        ) : null}
      </div>
    );
  }
