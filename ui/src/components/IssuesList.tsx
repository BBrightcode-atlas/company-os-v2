...
const quickFilterPresets = [
  { labelKey: "issuesList.all" as const, statuses: [] as string[] },
  { labelKey: "issuesList.active" as const, statuses: ["todo", "in_progress", "in_review", "blocked"] },
  { labelKey: "issuesList.backlog" as const, statuses: ["backlog"] },
  { labelKey: "issuesList.done" as const, statuses: ["done", "cancelled"] },
];
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className={`text-xs ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`}>
                ...
                <span className="hidden sm:inline">{activeFilterCount > 0 ? t("issuesList.filtersCount").replace("{count}", String(activeFilterCount)) : t("issuesList.filter")}</span>
                {activeFilterCount > 0 && (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                  ...
                  <div className="space-y-3">
                    ...
                    {teamsOption && teamsOption.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-xs text-muted-foreground">{t("issuesList.team")}</span>
                        <div className="space-y-0.5 max-h-32 overflow-y-auto">
                      ...
                      <IssueRow
                        ...
                        titleSuffix={hasChildren && !isExpanded ? (
                          <span className="ml-1.5 text-xs text-muted-foreground">
                            {totalDescendants !== 1 ? t("issuesList.subTasksPlural").replace("{count}", String(totalDescendants)) : t("issuesList.subTasks").replace("{count}", String(totalDescendants))}
                          </span>
                        ) : undefined}
                        mobileLeading={
                          hasChildren ? (
