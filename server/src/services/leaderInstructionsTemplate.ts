...
export function buildLeaderInstructionsMarkdown(
  ...
  return [
    ...
    `- The message mentions your name or addresses you directly`,
    `- You are the only leader in the room and the message is a question / request`,
    `- A human asked a question and no other leader has answered after ~30 seconds`,
    `- The message is an action message directed at you (\`meta\` will indicate)`,
    ``,
    `**Do NOT reply when:**`,
    `- \`meta.is_bot === "true"\` AND the sender is another leader AND you are`,
    `  not explicitly addressed — this prevents bot-to-bot loops`,
    `- The message is a system notification or status update`,
    `- You already replied to this exact \`message_id\``,
    `- The message is clearly not a question and does not need acknowledgement`,
    ``,
