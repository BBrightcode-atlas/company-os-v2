...
export function routineService(
  ...
  return {
    ...
    list: async (
      companyId: string,
      filters?: { teamId?: string | null; projectId?: string | null },
    ): Promise<RoutineListItem[]> => {
      const conditions = [eq(routines.companyId, companyId)];
      if (filters?.teamId) conditions.push(eq(routines.teamId, filters.teamId));
    ...
    create: async (companyId: string, input: CreateRoutine, actor: Actor): Promise<Routine> => {
      ...
      // routines keep the legacy "any active agent" rule. A null
      // assignee is allowed for draft routines (upstream behavior).
