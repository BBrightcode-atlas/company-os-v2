...
export function issueRoutes(
  ...
  async function assertCanAssignTasks(
    ...
    assertCompanyAccess(req, companyId);
    // COS: owner/admin 멤버십 사람 유저는 회사 내 할당을 명시 grant 없이
    // 수행할 수 있다 (PR #5). 그 외 모든 권한 판정은 upstream access.decide()에 위임.
  ...
  async function resolveIssueRouteId(rawId: string, req: any): Promise<string> {
  ...
  router.param("id", async (req, res, next, rawId) => {
    try {
      req.params.id = await resolveIssueRouteId(rawId, req);
      next();
    } catch (err) {
      next(err);
    }
  ...
  router.param("issueId", async (req, res, next, rawId) => {
    try {
      req.params.issueId = await resolveIssueRouteId(rawId, req);
      next();
    } catch (err) {
      next(err);
    }
