...
describeEmbeddedPostgres("low-trust red-team HTTP route regression suite", () => {
  ...
  it("restricts low-trust self inspection without changing standard-agent visibility", async () => {
    ...
    expect(standardRes.body).toHaveProperty("adapterConfig");
    expect(standardRes.body).toHaveProperty("runtimeConfig");
    expect(JSON.stringify(standardRes.body)).not.toContain(fixture.canaries.agentConfig);
    expect(JSON.stringify(standardRes.body)).toContain("***REDACTED***");
