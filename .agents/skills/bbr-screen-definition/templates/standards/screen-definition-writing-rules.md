...
9. Figma 원본 화면 기준으로 문서를 만들 때는 이 일반 룰이 아니라 `figma-screen-definition-writing-rules.md`와 Figma 전용 워크플로우를 사용한다.
...
- Figma 기준선 산출물과 feature 중심 구현 산출물을 같은 파일명 또는 같은 워크플로우로 섞지 않는다.
- 기능 코드를 새로 만들지 않는다.
