...
- non-trivial Feature, Plugin, UX Flow 작업이면 plugin flow design artifact가 있고, 그 안에 Flowchart와 Sequence Diagram이 모두 포함되어 있다. plugin repo에 더 엄격한 local convention이 없으면 Mermaid `flowchart TD`와 `sequenceDiagram`을 기본 형식으로 쓴다. capabilities, slots, worker/UI/API flow, actor handoff, error state가 바뀌면 두 diagram도 같은 변경에서 갱신한다.
...
- plugin-level Feature, Plugin, UX Flow 설계 문서에는 Flowchart와 Sequence Diagram을 포함하고, scope/flow 변경과 함께 최신 상태로 유지하며, 성공 보고 전 두 diagram의 존재 여부와 최신성을 검토한다
