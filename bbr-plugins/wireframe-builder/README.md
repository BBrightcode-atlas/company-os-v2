...
정제된 **개발 기획서 + 화면설계서**를 입력받아 **조작 가능한(clickable) 와이어프레임**을 생성·반복하는 BBR 전용 Paperclip 플러그인. `product-builder` 이전 단계로, 실제 개발 task 분해 전에 **화면 워크플로우를 미리 검증**한다.

- 입력: 개발 기획서(서술) + 화면설계서(화면 목록/구성/전환) — paste 또는 파일 첨부(md/txt/pdf)
- 출력: 화면 전환·모달·탭·조건부 표시가 실제로 동작하는 HTML 와이어프레임
- 수정: 자연어 채팅으로 "이 화면에 검색 추가" → 모델 patch → 재렌더

지금은 (1)단계·`product-builder` 와 분리된 **독립 self-contained** 플러그인이다.
...
| `src/contract.ts` | DB namespace(`plugin_wireframes_dfd3295c23`) + DATA/ACTION 키 + 타입 |
