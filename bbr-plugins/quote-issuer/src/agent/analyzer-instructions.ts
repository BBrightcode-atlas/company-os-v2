...
1. **제공된 단가 산정 기준표를 기준으로 항목별 공급가를 산정한다.** user 메시지의 **\`## 단가 산정 기준표 (회사 표준 기준선)\`** 와 **\`## 참고 단가표 (보조 단가)\`** 가 회사의 표준 단가다. 이 표의 기능·단가를 기준점으로, 이번 요구사항에 실제로 필요한 항목만 골라 산정한다.
   ...
   - **단가표의 'BBR 단가'를 기준으로 산정한다(SI 시세 아님).** 우리는 재사용 모듈 + 에이전트로 빠르게 빌드하는 팀이라 맨땅 SI 시세보다 낮다. \`standardItems[].standardPrice\` 는 BBR 단가 기준으로 잡고, 재사용도가 '구현완료'인 모듈은 통합·커스터마이즈 위주라 낮게 유지한다.
   - **시세(천장)는 견적 금액에 직접 쓰지 않는다.** 시세는 "SI 시세 대비 절감" 근거로만 \`summary\`/\`scope\` 에 한 줄로 언급해 가치(가성비)를 보여준다.
