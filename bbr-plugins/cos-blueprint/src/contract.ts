...
export const DATA = {
  overview: "overview",
  projects: "projects",
  projectDocumentSlots: "project-document-slots",
...
export type SourceDocumentRegisterResult = {
  ...
  projectId: string | null;
  workspacePath: string | null;
  /** 현재 호환 레이어에서 프로젝트 workspace에 기록한 파일 경로. 미기록 시 null. */
  file: string | null;
  ...
  if (hasUpload) {
    schemas.push({
      ...
      fields: [
        ...
        { ...field("storageKey", "string", true, "저장소 key"), validation: "workspace 상대 경로", example: "docs/cos-blueprint/sources/originals/proposal.docx" },
      ],
      relations: ["Attachment는 SourceMaterial 또는 WorkProduct에 연결된다."],
      acceptanceCriteria: ["원본 파일명과 storageKey가 모두 추적 가능해야 한다."],
    });
  }
  ...
  const apis: ApiDefinition[] = [
    {
      ...
      acceptanceCriteria: ["자료 등록 후 source 목록에 즉시 표시된다.", "프로젝트 선택 시 workspace source 문서가 생성된다."],
    },
    {
      code: "API-002",
      method: "POST",
...
// 등록 자료 1건이 현재 호환 레이어에서 적재될 프로젝트 workspace 파일 경로.
