...
15. 스키마 정의서는 Mermaid erDiagram을 최상단 기본 독해 지점으로 두고, ERD와 엔티티 상세 아래에서 REUSE/EXTEND/NEW/N/A 판정, product-builder-base 참고, migration scope를 설명한다. Drizzle table/export, 사람이 구현 가능한 테이블 컬럼 선언(fields name/type/required/description), relations, indexes/enums, implementation notes를 포함하고 undefined/null/빈 필드는 금지한다.
...
8. Mermaid erDiagram을 스키마 정의서 최상단의 기본 독해 지점으로 둔다. 스키마 참고/재활용(product-builder-base, REUSE/EXTEND/NEW/N/A, migration scope)은 ERD와 엔티티 상세 아래 설명 섹션에서 읽히도록 분리한다. relations는 \`A 1:N B\`, \`A N:1 B\`, \`fieldId -> target.id\`처럼 ERD 관계로 변환 가능한 표현으로 남긴다.
