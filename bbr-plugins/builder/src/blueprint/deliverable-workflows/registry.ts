...
export const BLUEPRINT_DELIVERABLE_WORKFLOW_DEFINITIONS: readonly BlueprintDeliverableWorkflowDefinition[] = [
  ...
  },
  {
    ...
    phase: "contract",
    description: "기능 목록과 기능별 상세 문서를 관리자(admin), 웹서비스(site), 앱(app), 랜딩(landing) 구획으로 나누고 project-builder-base 재사용 판정을 기록한다.",
  },
  {
    ...
    owner: "Contract Agent",
    phase: "contract",
    description: "기능정의서의 feature cluster와 product-builder-base packages/drizzle 재사용 후보를 기준으로 데이터 객체, 필드, 관계, 검증 규칙을 계약화한다.",
  },
  {
  ...
  },
  {
    ...
    owner: "Screen Agent",
    phase: "screen",
    description: "개발 요구사항 브리프 확정 뒤 관리자(admin), 웹서비스(site), 앱(app), 랜딩(landing) 화면을 분리해 화면 단위 계약과 QA 기준을 작성한다.",
  },
  {
