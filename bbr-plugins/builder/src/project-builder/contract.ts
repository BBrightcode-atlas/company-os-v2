...
export const DATA = {
  overview: "project-builder.overview",
  blueprint: "project-builder.blueprint",
} as const;
...
export type ProductBuilderProjectBlueprintSelection = {
  projectId: string;
  blueprintId: string;
  displayName: string;
...
export type ProductBuilderUpstreamSlotKey = typeof PRODUCT_BUILDER_REQUIRED_UPSTREAM_SLOT_KEYS[number];
export type ProductBuilderUpstreamSlotReadiness = {
  slotKey: ProductBuilderUpstreamSlotKey;
  title: string;
  status: ProductBuilderDeliverableSlotStatus | "missing";
  requiredStatus: "ready-or-approved" | "ready-approved-or-na";
  hasContent: boolean;
  ready: boolean;
  checkedAt: string;
};

export type ProductBuilderUpstreamReadiness = {
  projectId: string | null;
  ready: boolean;
  checkedAt: string;
  slots: ProductBuilderUpstreamSlotReadiness[];
};
...
export const ONLINE_SERVICE_BLUEPRINT: ProductBuilderBlueprint = {
  ...
  tasks: [
    ...
    pbTask({
      key: "PB-INFRA-002",
      phase: "05 도메인 기반",
      title: "Neon migration/DB 운영 준비 검증",
      description: "Neon 개발/스테이징 브랜치에 migration과 seed를 적용하고, 앱 서버가 실제 DATABASE_URL로 연결되는지 health query로 검증한다.",
      category: "ops",
      priority: "high",
      capabilityKey: "environment.neon-migration-readiness",
      agentRole: "Platform Engineer",
      surfaces: ["api", "ops"],
    ...
    pbTask({
      ...
      deliverables: ["KCB 테스트 계정 증거", "JAR/JVM adapter health 증거", "성공/실패/취소/만료 E2E", "callback 보안 테스트", "개인정보 저장 검증", "관리자 UI 증거", "배포 URL 운영 준비 증거"],
      acceptanceCriteria: [
    ...
    pbTask({
      ...
      description: "Polar hosted checkout 생성, product/price id mapping, success URL 템플릿 변수, idempotency key, coupon/discount 전달을 구현한다. Polar 미선택 시 N/A로 SKIP한다.",
      decision: "N/A",
      category: "backend",
      priority: "high",
      ...
      acceptanceCriteria: [
        ...
        "Polar API snake_case와 템플릿 변수 규칙이 테스트로 보호된다.",
      ],
    }),
    pbTask({
    ...
    pbTask({
      ...
      description: `고객 사업자와 KG이니시스 계약/심사, MID, signKey, API key, 가상계좌 사용 여부, 모바일/PC 표준결제 방식, 테스트/운영 상점 설정을 확정한다. 최초 개발 시 일반결제는 공식 매뉴얼 ${INICIS_STDPAY_PC_MANUAL_URL}, 정기결제는 공식 매뉴얼 ${INICIS_BILLING_MANUAL_URL}을 기준으로 하고 추론/블로그/비공식 예시 기억 기반 구현을 금지한다. INICIS 미선택 시 N/A로 SKIP한다.`,
      decision: "N/A",
      category: "ops",
      priority: "critical",
    ...
    pbTask({
      ...
      acceptanceCriteria: [
        ...
        "배포 URL의 health check 또는 동등한 readiness endpoint가 통과한다.",
      ],
    }),
    pbTask({
      key: "PB-LAUNCH-SMOKE-001",
      phase: "14 배포/운영",
      title: "실서비스 로그인 운영 준비 검증",
      description: "최종 배포 URL에서 공개 탐색, auth modal, 회원가입/로그인, 보호 기능 진입, 관리자 접근 제어를 실제 브라우저로 검증한다.",
      category: "qa",
      priority: "critical",
      capabilityKey: "qa.production-login-readiness",
      agentRole: "QA Engineer",
      ...
      deliverables: ["브라우저 운영 준비 검증 결과", "로그인 계정/테스트 데이터", "스크린샷 또는 녹화", "잔여 이슈"],
      acceptanceCriteria: [
...
export const WEB_APPLICATION_SERVICE_BLUEPRINT: ProductBuilderBlueprint = {
  ...
  constraints: [
    ...
    "실제 납품 완료는 Vercel 배포 URL에서 로그인, 보호 기능, 관리자 접근 제어, AI 서버 운영 준비 상태가 검증되어야 한다.",
...
export function renderBuildPlanMarkdown(input: ProductBuilderDocumentRenderInput): string {
  ...
  return [
    ...
    "| --- | --- | --- |",
    "| 개발 요구사항 브리프(Development Requirements Brief) | `deliverable.prd` | 구현 범위, 기능 요구사항, 인수 기준 확인 |",
