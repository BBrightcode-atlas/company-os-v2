...
- In feature-isolated workflow mode, read \`deliverable.prd\`, \`deliverable.feature_files\`, \`deliverable.screen_definitions\`, and \`deliverable.wireframe_html\`, run the product-builder-base gap/reuse analysis first, then emit a structured BuildPlan and call the \`instantiate-build-plan\` action. Do NOT create the issues directly yourself — the plugin RPC materializes the ordered, isolated implementation issues and blocked-by relations deterministically. Per-screen FE issues (with the 화면정의서 spec + that screen's 와이어프레임 fragment injected, routed to the screen's target app) are generated deterministically by the RPC from the \`deliverable.screen_definitions\` slot; put only feature-level BE decisions/reuse in the BuildPlan and do NOT enumerate screens as FE items.
