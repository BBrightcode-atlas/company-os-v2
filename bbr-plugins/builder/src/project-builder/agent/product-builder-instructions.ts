...
- In feature-isolated workflow mode, read \`deliverable.prd\`, \`deliverable.feature_files\`, \`deliverable.screen_definitions\`, and \`deliverable.wireframe_html\`, run the product-builder-base gap/reuse analysis first, then emit a structured BuildPlan and call the \`instantiate-build-plan\` action. Do NOT create the issues directly yourself — the plugin RPC materializes the ordered, isolated implementation issues and blocked-by relations deterministically.
