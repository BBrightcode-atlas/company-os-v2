...
| 2 | 개발 요구사항 브리프 생성(Development Requirements Brief) | Source slots | 개발 요구사항 브리프 | 확정 구현 범위, 기능 요구사항, 인수 기준, 제외 범위가 검증 가능함 | PM Agent |
| 3 | 기능 정의서 생성(Feature Definition) | 개발 요구사항 브리프 | 기능정의서(표면별 목록 페이지 + 기능별 상세 문서 + base 재사용 판정) | 기능명, 설정에서 선택된 apps/admin, apps/site, apps/app, apps/landing surface, Project slot 문서 참조, project-builder-base 재사용 판정으로 추적 가능함 | PM Agent |
| 4 | 계약 문서 생성(Contract Definition) | 기능 정의서 | 스키마/API 정의서 | 개발/QA가 참조 가능한 계약이 있음 | Contract Agent |
| 5 | 화면정의서 생성(Screen Definition) | 개발 요구사항 브리프, 기능 정의서, 계약 문서 | 표면별 화면 목록 + 화면별 화면정의서 | 주요 화면, 설정에서 선택된 apps/admin, apps/site, apps/app, apps/landing surface, 상태, 액션, API 참조, 페이지별 layout/slot이 있음 | Screen Agent |
...
- 기능 정의서는 project-builder-base를 기본 코드베이스로 전제하고, 기능별로 설정에서 선택된 apps/admin, apps/site, apps/app, apps/landing 대상 surface와 전체 재사용/부분 재사용/커스터마이징/신규 판정을 남긴다.
- 화면정의서는 설정에서 선택된 apps/admin, apps/site, apps/app, apps/landing을 섞지 않고 surface별 목록과 화면별 문서로 남긴다.
