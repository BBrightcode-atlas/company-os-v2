...
2. 산출물 분해표(Output Inventory)를 먼저 만들고, 이를 기준으로 표준 기획서(Standard Plan), PRD(Product Requirements Document), 기능 정의서(Feature Definition), 스키마/API/인터페이스/레이아웃/화면정의서를 고정 산출물로 만든다.
...
flowchart LR
  subgraph BP["1. Blueprint"]
    direction LR
    B1["<b>register-source-document</b><br/>고객 자료 등록·중복 제거"] --> B2["<b>set-product-builder-blueprint</b><br/>제품 유형 선택"] --> B3["<b>run-requirement-inventory</b><br/>산출물별 작성 단위 분해"] --> B4["<b>run-standard-plan</b><br/>분해표 기준 표준 기획서 생성"] --> B5["<b>confirm-standard-plan</b><br/>기획서 확정 게이트"] --> B6["<b>write-standard-plan-docs</b><br/>기획 문서 slot 기록"] --> B7["<b>run-screens</b><br/>화면정의서 LLM 생성"] --> B8["<b>review / regenerate-screen</b><br/>화면 검수·재생성"] --> B9["<b>write-screen-docs</b><br/>화면정의서 slot 기록"]
  end
...
sequenceDiagram
  ...
  W->>L: POST /v1/messages (output-inventory chunk prompt)
  L-->>W: JSON items + targetDeliverables (실패/요약 누락 대비 deterministic fallback item 병합)
...
| `run-requirement-inventory` | 등록 자료를 산출물 분해표(Output Inventory)로 먼저 분해 |
| `run-standard-plan` | 산출물 분해표 기준 표준 기획서/PRD/기능/계약 산출물 초안 생성 |
...
| `deliverable.requirement_inventory` | 산출물 분해표(Output Inventory) | `templates/deliverables/output-inventory.md` |
...
| Slot | 입력(Input) |
| --- | --- |
| `deliverable.requirement_inventory` | 산출물 분해표(Output Inventory) |
...
- Blueprint 산출물 분해표가 표준 기획서 이전에 생성되고 Project slot/wiki/Product Builder upstream gate에 연결되는지
