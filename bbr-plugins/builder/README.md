...
2. 요구사항 목록(Requirement Inventory)을 먼저 만들고, 이를 기준으로 표준 기획서(Standard Plan), PRD(Product Requirements Document), 기능 정의서(Feature Definition), 스키마/API/인터페이스/레이아웃/화면정의서를 고정 산출물로 만든다.
...
flowchart LR
  subgraph BP["1. Blueprint"]
    direction LR
    B1["<b>register-source-document</b><br/>고객 자료 등록·중복 제거"] --> B2["<b>set-product-builder-blueprint</b><br/>제품 유형 선택"] --> B3["<b>run-requirement-inventory</b><br/>자료별 atomic item 목록화"] --> B4["<b>run-standard-plan</b><br/>목록 기준 표준 기획서 생성"] --> B5["<b>confirm-standard-plan</b><br/>기획서 확정 게이트"] --> B6["<b>write-standard-plan-docs</b><br/>기획 문서 slot 기록"] --> B7["<b>run-screens</b><br/>화면정의서 LLM 생성"] --> B8["<b>review / regenerate-screen</b><br/>화면 검수·재생성"] --> B9["<b>write-screen-docs</b><br/>화면정의서 slot 기록"]
  end
...
| Blueprint | `run-requirement-inventory` | 등록 자료를 source-backed atomic item으로 목록화. 긴 문서는 chunk 단위로 분석하고 deterministic coverage floor로 누락을 줄임 |
| Blueprint | `run-standard-plan` | 요구사항 목록을 기준으로 표준 기획서(목표·범위·스키마·API·레이아웃) LLM 생성. fire-and-forget job |
...
| Project Builder | `blueprint.overview` | 필수 upstream slot(requirement_inventory·plan·prd·schema·api·screens·wireframe…) ready/approved 검증 |
...
sequenceDiagram
  ...
  W->>L: POST /v1/messages (requirement-inventory chunk prompt)
  L-->>W: JSON items (실패/요약 누락 대비 deterministic fallback item 병합)
  W->>S: state lock + jobId 일치 시 requirementInventory 저장, standardPlan/screenPlan 무효화, job=null
  UI->>W: run-standard-plan(companyId, projectId, title)
  W->>S: readState → requirementInventory 확인(없으면 먼저 생성)
  W->>S: startJob → job=running (state lock)
  W->>L: POST /v1/messages (standard-plan prompt)
  L-->>W: JSON (실패 시 deterministic fallback)
  W->>S: inventory item coverage 보정 후 standardPlan 저장, screenPlan 무효화, job=null
  loop 폴링
...
sequenceDiagram
  ...
  W->>SL: 필수 upstream slot readiness 확인 (requirement_inventory/plan/prd/schema/api/screens/wireframe ...)
  alt 미충족
...
| `run-requirement-inventory` | 등록 자료를 요구사항 목록(Requirement Inventory)으로 먼저 분해 |
| `run-standard-plan` | 요구사항 목록 기준 표준 기획서/PRD/기능/계약 산출물 초안 생성 |
...
| `deliverable.requirement_inventory` | 요구사항 목록(Requirement Inventory) | `templates/deliverables/requirement-inventory.md` |
...
- 요구사항 목록(Requirement Inventory)은 표준 기획서 이전에 생성되는 source-backed coverage baseline이다.
- 기능/권한/화면 후보/데이터/API/관리자 작업/결제/알림/업로드/AI runtime/비기능/리스크/오픈 질문을 대표 요약이 아니라 atomic item으로 남긴다.
...
| Slot | 입력(Input) |
| --- | --- |
| `deliverable.requirement_inventory` | 요구사항 목록(Requirement Inventory) |
...
- Blueprint 요구사항 목록이 표준 기획서 이전에 생성되고 Project slot/wiki/Product Builder upstream gate에 연결되는지
