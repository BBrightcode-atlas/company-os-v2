# Figma 화면정의서(Figma Screen Definition) - SITE-SCR-009 명의 찾기
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 명의 찾기 |
| 화면 코드(Screen Code) | SITE-SCR-009 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 명의 찾기 검색/목록 화면군. Figma node 4390:13162. |
| 경로(Route) | /doctors |
| 인증/권한(Auth/Permission) | guest_or_member |
| 레이아웃(Layout) | Figma mobile site frame with AIGA design system tokens |
| Primary test-id | site-scr-009 |
...
| 항목(Item) | 내용(Description) |
| --- | --- |
| Figma 파일 URL | https://www.figma.com/design/4F1yav1LVfMJFluyIQq7pr/AIGA-Official-External?node-id=4390-13162&t=OWrLX26n9Z69PyHu-4 |
| Figma file key | 4F1yav1LVfMJFluyIQq7pr |
| Figma node id | 4390-13162 / 4390:13162 |
| Frame/Page name | 명의 찾기 |
...
| --- | --- | --- |
| Figma Frame | 명의 찾기 | supplied node 4390:13162 |
...
| --- | --- | --- | --- |
| Default | 해당 Figma 프레임의 기본 표시 상태를 기준으로 구현한다. | 4390:13162 | 기본 진입 또는 조회 성공 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | site-scr-009-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-007 | 현재 화면/상태 유지 |
| 주요 액션 실행 | site-scr-009-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-008, API-009, API-010 | 현재 화면/상태 유지 |
| 상태 복구 또는 이동 | site-scr-009-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | 현재 화면/상태 유지 |
...
~~~mermaid
flowchart TD
  Entry[명의 찾기 Figma node 진입] --> Visual[Figma MCP screenshot/metadata 확인]
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-009 명의 찾기
  participant Figma as Figma node 4390:13162
...
| --- | --- | --- | --- | --- | --- | --- |
| 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | 권한/데이터 fixture 준비 | 화면 진입 | 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | state/action/API mock | site-scr-009-ac-01 | Playwright candidate |
| 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | 권한/데이터 fixture 준비 | 주요 액션 실행 | 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | state/action/API mock | site-scr-009-ac-02 | Playwright candidate |
| 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | 권한/데이터 fixture 준비 | 권한 필요 액션 실행 | 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | state/action/API mock | site-scr-009-ac-03 | Playwright candidate |
