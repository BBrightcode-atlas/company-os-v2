# Figma 화면정의서(Figma Screen Definition) - SITE-SCR-016 의사 본인 인증#3
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 의사 본인 인증#3 |
| 화면 코드(Screen Code) | SITE-SCR-016 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 의사 본인 인증 플로우 화면군. Figma node 4390:13384. |
...
| 항목(Item) | 내용(Description) |
| --- | --- |
| Figma 파일 URL | https://www.figma.com/design/4F1yav1LVfMJFluyIQq7pr/AIGA-Official-External?node-id=4390-13384&t=OWrLX26n9Z69PyHu-4 |
| Figma file key | 4F1yav1LVfMJFluyIQq7pr |
| Figma node id | 4390-13384 / 4390:13384 |
| Frame/Page name | 의사 본인 인증#3 |
...
| --- | --- | --- |
| Figma Frame | 의사 본인 인증#3 | supplied node 4390:13384 |
...
| --- | --- | --- | --- |
| Default | 해당 Figma 프레임의 기본 표시 상태를 기준으로 구현한다. | 4390:13384 | 기본 진입 또는 조회 성공 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | site-scr-016-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-013 | 현재 화면/상태 유지 |
| 주요 액션 실행 | site-scr-016-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-014, API-015, API-016 | 현재 화면/상태 유지 |
...
~~~mermaid
flowchart TD
  Entry[의사 본인 인증#3 Figma node 진입] --> Visual[Figma MCP screenshot/metadata 확인]
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-016 의사 본인 인증#3
  participant Figma as Figma node 4390:13384
