# Figma 화면정의서(Figma Screen Definition) - SITE-SCR-031 의사 프로필 편집
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 의사 프로필 편집 |
| 화면 코드(Screen Code) | SITE-SCR-031 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 의사 프로필 편집 Figma node. 인증 의사가 본인 프로필의 편집 가능 영역을 수정하거나 정보 수정 요청으로 연결한다. |
| 경로(Route) | /my/doctor-profile/edit |
| 인증/권한(Auth/Permission) | doctor_verified_member |
| 레이아웃(Layout) | Figma mobile site frame with AIGA design system tokens |
| Primary test-id | site-scr-031 |
...
| 항목(Item) | 내용(Description) |
| --- | --- |
| Figma 파일 URL | https://www.figma.com/design/4F1yav1LVfMJFluyIQq7pr/AIGA-Official-External?node-id=4390-14001&t=OWrLX26n9Z69PyHu-4 |
| Figma file key | 4F1yav1LVfMJFluyIQq7pr |
| Figma node id | 4390-14001 / 4390:14001 |
| Frame/Page name | 의사 프로필 편집 |
...
| --- | --- | --- |
| Grid/Container | mobile 375px frame, section/card/list/form 구조 | doctor profile edit form, editable sections, image upload, change request CTA |
...
| --- | --- |
| 스키마(Schema) | SCH-001, SCH-002, SCH-003, SCH-009, SCH-004, SCH-013, SCH-005, SCH-020, SCH-024, SCH-025, SCH-022 |
| API | API-002 GET /api/me - 현재 사용자 세션과 등급 조회<br>API-011 GET /api/doctors/{doctorProfileId} - 의사 프로필 상세<br>API-012 POST /api/doctors/{doctorProfileId}/change-requests - 의사 정보 수정 요청<br>API-017 GET /api/doctor-verifications/me - 내 의사 인증 상태 조회<br>API-079 POST /api/files/upload-url - 파일 업로드 URL 생성<br>API-080 POST /api/files/confirm - 파일 업로드 확정<br>API-081 GET /api/files/{fileId}/signed-url - 파일 signed URL 조회<br>API-086 POST /api/analytics/events - 분석 이벤트 수집 |
| 기능 정의서(Feature Definition) | FEA-008 의사 인증 플로우와 면허번호 검증<br>FEA-009 의사 인증 표시와 실명 전환<br>FEA-010 의사 프로필, 명의 찾기, 정보 수정 요청<br>FEA-012 마이페이지, 내 활동, 고객지원, 회원탈퇴<br>FEA-022 개인정보, 접근성, 분석 이벤트 |

## 4. 화면 구성(Screen Composition)

| 영역(Area) | 컴포넌트(Component) | 표시 조건(Display Rule) |
| --- | --- | --- |
| Figma Frame | 의사 프로필 편집 | supplied node 4390:14001 |
| Primary Content | doctor profile edit form, editable sections, image upload, change request CTA | Figma screenshot에서 확인된 핵심 화면 구조 |
...
| API-bound content | display list/detail | Conditional | API-002, API-011, API-012, API-017, API-079, API-080, API-081, API-086 | 선행 API 계약 참조 |

## 6. 화면 상태(Screen States)

| 상태(State) | 정의(Definition) | Figma node | 표시 조건(Display Rule) |
| --- | --- | --- | --- |
| Default | 해당 Figma 프레임의 기본 표시 상태를 기준으로 구현한다. | 4390:14001 | 기본 진입 또는 조회 성공 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | site-scr-031-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-001 | 현재 화면/상태 유지 |
| 주요 액션 실행 | site-scr-031-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-007, API-008, API-009 | 현재 화면/상태 유지 |
| 상태 복구 또는 이동 | site-scr-031-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | 현재 화면/상태 유지 |
...
~~~mermaid
flowchart TD
  Entry[의사 프로필 편집 Figma node 진입] --> Visual[Figma MCP screenshot/metadata 확인]
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-031 의사 프로필 편집
  participant Figma as Figma node 4390:14001
...
| --- | --- | --- | --- | --- | --- | --- |
| 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | 권한/데이터 fixture 준비 | 화면 진입 | 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | state/action/API mock | site-scr-031-ac-01 | Playwright candidate |
| 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | 권한/데이터 fixture 준비 | 주요 액션 실행 | 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | state/action/API mock | site-scr-031-ac-02 | Playwright candidate |
| 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | 권한/데이터 fixture 준비 | 권한 필요 액션 실행 | 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | state/action/API mock | site-scr-031-ac-03 | Playwright candidate |
