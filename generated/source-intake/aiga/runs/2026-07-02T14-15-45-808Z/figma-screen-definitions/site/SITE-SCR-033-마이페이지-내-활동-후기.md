# Figma 화면정의서(Figma Screen Definition) - SITE-SCR-033 마이페이지-내 활동-후기
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 마이페이지-내 활동-후기 |
| 화면 코드(Screen Code) | SITE-SCR-033 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 내 활동 후기 탭 Figma node. 의사 후기 작성 이력과 상세 이동/삭제 진입을 표시한다. |
| 경로(Route) | /my/activity |
| 인증/권한(Auth/Permission) | guest_or_member |
| 레이아웃(Layout) | Figma mobile site frame with AIGA design system tokens |
| Primary test-id | site-scr-033 |
...
| 항목(Item) | 내용(Description) |
| --- | --- |
| Figma 파일 URL | https://www.figma.com/design/4F1yav1LVfMJFluyIQq7pr/AIGA-Official-External?node-id=4390-14142&t=OWrLX26n9Z69PyHu-4 |
| Figma file key | 4F1yav1LVfMJFluyIQq7pr |
| Figma node id | 4390-14142 / 4390:14142 |
| Frame/Page name | 마이페이지-내 활동-후기 |
...
| --- | --- | --- |
| Grid/Container | mobile 375px frame, section/card/list/form 구조 | profile summary, activity tabs, review activity list, bottom GNB |
...
| --- | --- |
| 스키마(Schema) | SCH-003, SCH-009, SCH-010, SCH-019, SCH-014, SCH-013, SCH-006, SCH-007, SCH-021, SCH-022 |
| API | API-034 GET /api/doctors/{doctorProfileId}/reviews - 의사 후기 목록<br>API-037 DELETE /api/reviews/{reviewId} - 의사 후기 삭제<br>API-082 GET /api/me/activity - 내 활동 집계 조회<br>API-086 POST /api/analytics/events - 분석 이벤트 수집 |
| 기능 정의서(Feature Definition) | FEA-006 의사 리뷰와 후기 작성<br>FEA-012 마이페이지, 내 활동, 고객지원, 회원탈퇴<br>FEA-022 개인정보, 접근성, 분석 이벤트 |

## 4. 화면 구성(Screen Composition)

| 영역(Area) | 컴포넌트(Component) | 표시 조건(Display Rule) |
| --- | --- | --- |
| Figma Frame | 마이페이지-내 활동-후기 | supplied node 4390:14142 |
| Primary Content | profile summary, activity tabs, review activity list, bottom GNB | Figma screenshot에서 확인된 핵심 화면 구조 |
...
| API-bound content | display list/detail | Conditional | API-034, API-037, API-082, API-086 | 선행 API 계약 참조 |

## 6. 화면 상태(Screen States)

| 상태(State) | 정의(Definition) | Figma node | 표시 조건(Display Rule) |
| --- | --- | --- | --- |
| Default | 해당 Figma 프레임의 기본 표시 상태를 기준으로 구현한다. | 4390:14142 | 기본 진입 또는 조회 성공 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | site-scr-033-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-001 | 현재 화면/상태 유지 |
| 주요 액션 실행 | site-scr-033-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-002, API-003, API-005 | 현재 화면/상태 유지 |
| 상태 복구 또는 이동 | site-scr-033-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | 현재 화면/상태 유지 |
...
~~~mermaid
flowchart TD
  Entry[마이페이지-내 활동-후기 Figma node 진입] --> Visual[Figma MCP screenshot/metadata 확인]
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-033 마이페이지-내 활동-후기
  participant Figma as Figma node 4390:14142
...
| --- | --- | --- | --- | --- | --- | --- |
| 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | 권한/데이터 fixture 준비 | 화면 진입 | 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | state/action/API mock | site-scr-033-ac-01 | Playwright candidate |
| 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | 권한/데이터 fixture 준비 | 주요 액션 실행 | 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | state/action/API mock | site-scr-033-ac-02 | Playwright candidate |
| 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | 권한/데이터 fixture 준비 | 권한 필요 액션 실행 | 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | state/action/API mock | site-scr-033-ac-03 | Playwright candidate |
