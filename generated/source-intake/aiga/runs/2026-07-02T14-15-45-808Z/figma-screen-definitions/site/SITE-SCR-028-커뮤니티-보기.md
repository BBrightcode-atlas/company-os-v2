# Figma 화면정의서(Figma Screen Definition) - SITE-SCR-028 커뮤니티 보기
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 커뮤니티 보기 |
| 화면 코드(Screen Code) | SITE-SCR-028 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 커뮤니티 게시글 상세 화면군. Figma node 4390:13751. |
| 경로(Route) | /community/posts/:postId |
| 인증/권한(Auth/Permission) | guest_or_member |
| 레이아웃(Layout) | Figma mobile site frame with AIGA design system tokens |
| Primary test-id | site-scr-028 |
...
| 항목(Item) | 내용(Description) |
| --- | --- |
| Figma 파일 URL | https://www.figma.com/design/4F1yav1LVfMJFluyIQq7pr/AIGA-Official-External?node-id=4390-13751&t=OWrLX26n9Z69PyHu-4 |
| Figma file key | 4F1yav1LVfMJFluyIQq7pr |
| Figma node id | 4390-13751 / 4390:13751 |
| Frame/Page name | 커뮤니티 보기 |
| Viewport/Frame size | 공급 노드는 mobile 375px frame 계열이다. 대표 확인: 커뮤니티 보기 4390:13751 original 375x3262; observed post detail, image/content body, comments/replies, bottom actions. |
...
| --- | --- | --- |
| Grid/Container | mobile 375px frame, section/card/list/form 구조 | post detail, image/content body, comments, replies, bottom actions |
...
| --- | --- | --- | --- |
| post detail, image/content body, comments, replies, bottom actions | site feature component | Default/variant by node title | Figma MCP screenshot에서 확인한 구조 |
...
| --- | --- |
| 스키마(Schema) | SCH-002, SCH-006, SCH-007, SCH-008, SCH-012, SCH-014, SCH-013, SCH-001, SCH-016, SCH-018 |
| API | API-022 GET /api/community/posts/{postId} - 커뮤니티 게시글 상세<br>API-025 DELETE /api/community/posts/{postId} - 커뮤니티 게시글 삭제<br>API-026 GET /api/community/posts/{postId}/comments - 커뮤니티 게시글 댓글 목록<br>API-027 POST /api/community/comments - 커뮤니티 댓글 작성<br>API-028 PUT /api/community/comments/{commentId} - 커뮤니티 댓글 수정<br>API-029 DELETE /api/community/comments/{commentId} - 커뮤니티 댓글 삭제<br>API-030 POST /api/community/votes - 커뮤니티 공감 등록<br>API-031 DELETE /api/community/votes - 커뮤니티 공감 취소<br>API-032 POST /api/community/reports - 커뮤니티 콘텐츠 신고 |
| 기능 정의서(Feature Definition) | FEA-003 비회원 제한, 한도, 로그인 유도<br>FEA-004 커뮤니티 피드, 글쓰기, 상세, 임시저장<br>FEA-005 댓글, 대댓글, 공감 인터랙션<br>FEA-009 의사 인증 표시와 실명 전환<br>FEA-014 신고, 블라인드, 운영 처리<br>FEA-015 콘텐츠 안전 자동 차단<br>FEA-021 운영 정책 설정<br>FEA-022 개인정보, 접근성, 분석 이벤트 |

## 4. 화면 구성(Screen Composition)

| 영역(Area) | 컴포넌트(Component) | 표시 조건(Display Rule) |
| --- | --- | --- |
| Figma Frame | 커뮤니티 보기 | supplied node 4390:13751 |
| Primary Content | post detail, image/content body, comments, replies, bottom actions | Figma screenshot에서 확인된 핵심 화면 구조 |
...
| API-bound content | display list/detail | Conditional | API-022, API-025, API-026, API-027, API-028, API-029, API-030, API-031, API-032 | 선행 API 계약 참조 |

## 6. 화면 상태(Screen States)

| 상태(State) | 정의(Definition) | Figma node | 표시 조건(Display Rule) |
| --- | --- | --- | --- |
| Default | 해당 Figma 프레임의 기본 표시 상태를 기준으로 구현한다. | 4390:13751 | 기본 진입 또는 조회 성공 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | site-scr-028-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-022 | 현재 화면/상태 유지 |
| 주요 액션 실행 | site-scr-028-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-025, API-026, API-027 | 현재 화면/상태 유지 |
| 상태 복구 또는 이동 | site-scr-028-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | 현재 화면/상태 유지 |
...
~~~mermaid
flowchart TD
  Entry[커뮤니티 보기 Figma node 진입] --> Visual[Figma MCP screenshot/metadata 확인]
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-028 커뮤니티 보기
  participant Figma as Figma node 4390:13751
  participant API as API-022
...
| --- | --- | --- | --- | --- | --- | --- |
| 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | 권한/데이터 fixture 준비 | 화면 진입 | 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | state/action/API mock | site-scr-028-ac-01 | Playwright candidate |
| 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | 권한/데이터 fixture 준비 | 주요 액션 실행 | 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | state/action/API mock | site-scr-028-ac-02 | Playwright candidate |
| 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | 권한/데이터 fixture 준비 | 권한 필요 액션 실행 | 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | state/action/API mock | site-scr-028-ac-03 | Playwright candidate |
