# 화면정의서(Screen Definition) - SITE-SCR-006 리뷰 작성 및 수정
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 리뷰 작성 및 수정 |
| 화면 코드(Screen Code) | SITE-SCR-006 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 별점, 본문, 이미지, 병원 방문 인증 옵션을 포함한 의사 리뷰 작성/수정 화면이다. |
| 경로(Route) | /doctors/:doctorProfileId/reviews/new |
| 인증/권한(Auth/Permission) | member_except_self_doctor_profile |
...
| --- | --- |
| 스키마(Schema) | SCH-009, SCH-010, SCH-016, SCH-001, SCH-003, SCH-014, SCH-020, SCH-024, SCH-025, SCH-022 |
| API | API-018 POST /api/medical-visit-verifications - 병원 방문 인증 제출<br>API-019 GET /api/medical-visit-verifications/{verificationId} - 병원 방문 인증 상태 조회<br>API-035 POST /api/doctors/{doctorProfileId}/reviews - 의사 후기 작성<br>API-036 PATCH /api/reviews/{reviewId} - 의사 후기 수정<br>API-048 GET /api/operational-policies/effective - 적용 중 운영 정책 조회<br>API-079 POST /api/files/upload-url - 파일 업로드 URL 생성<br>API-080 POST /api/files/confirm - 파일 업로드 확정<br>API-081 GET /api/files/{fileId}/signed-url - 파일 signed URL 조회<br>API-086 POST /api/analytics/events - 분석 이벤트 수집 |
| 기능 정의서(Feature Definition) | FEA-006 의사 리뷰와 후기 작성<br>FEA-007 병원 방문 인증과 증빙 뱃지<br>FEA-014 신고, 블라인드, 운영 처리<br>FEA-015 콘텐츠 안전 자동 차단<br>FEA-021 운영 정책 설정<br>FEA-022 개인정보, 접근성, 분석 이벤트 |
...
| Main Content | 리뷰 작성 및 수정 primary content | 권한 허용 및 조회 성공 시 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | site-scr-006-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-018 | 현재 화면/상태 유지 |
| 주요 액션 실행 | site-scr-006-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-019, API-035, API-036 | SITE-SCR-005 |
| 상태 복구 또는 이동 | site-scr-006-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | SITE-SCR-005 |
...
~~~mermaid
flowchart TD
  Entry[/doctors/:doctorProfileId/reviews/new 진입] --> Auth{권한/세션 확인}
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-006 리뷰 작성 및 수정
