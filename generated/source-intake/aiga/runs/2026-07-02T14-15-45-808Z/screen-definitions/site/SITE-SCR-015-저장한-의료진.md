# 화면정의서(Screen Definition) - SITE-SCR-015 저장한 의료진
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 저장한 의료진 |
| 화면 코드(Screen Code) | SITE-SCR-015 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 저장한 의사 목록과 저장 취소, 프로필 상세 이동을 제공한다. |
| 경로(Route) | /my/saved-doctors |
| 인증/권한(Auth/Permission) | member |
| 레이아웃(Layout) | apps/site mobile shell + bottom GNB where applicable |
| Primary test-id | site-scr-015 |

## 2. 참조 계약(Referenced Contracts)

| 구분(Type) | 참조(Reference) |
| --- | --- |
| 스키마(Schema) | SCH-002, SCH-003, SCH-009, SCH-021 |
| API | API-011 GET /api/doctors/{doctorProfileId} - 의사 프로필 상세<br>API-083 GET /api/me/saved-doctors - 저장한 의료진 목록<br>API-084 POST /api/doctors/{doctorProfileId}/save - 의사 프로필 저장<br>API-085 DELETE /api/doctors/{doctorProfileId}/save - 의사 프로필 저장 취소 |
...
| Main Content | 저장한 의료진 primary content | 권한 허용 및 조회 성공 시 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | site-scr-015-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-011 | 현재 화면/상태 유지 |
| 주요 액션 실행 | site-scr-015-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-083, API-084, API-085 | SITE-SCR-005 |
| 상태 복구 또는 이동 | site-scr-015-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | SITE-SCR-005 |
...
~~~mermaid
flowchart TD
  Entry[/my/saved-doctors 진입] --> Auth{권한/세션 확인}
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-015 저장한 의료진
  participant API as API-011
...
| --- | --- | --- | --- | --- | --- | --- |
| 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | 권한/데이터 fixture 준비 | 화면 진입 | 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | state/action/API mock | site-scr-015-ac-01 | Playwright candidate |
| 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | 권한/데이터 fixture 준비 | 주요 액션 실행 | 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | state/action/API mock | site-scr-015-ac-02 | Playwright candidate |
| 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | 권한/데이터 fixture 준비 | 권한 필요 액션 실행 | 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | state/action/API mock | site-scr-015-ac-03 | Playwright candidate |
