# 화면정의서(Screen Definition) - SITE-SCR-016 고객지원 공지와 정책 콘텐츠
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 고객지원 공지와 정책 콘텐츠 |
| 화면 코드(Screen Code) | SITE-SCR-016 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 공지사항 목록/상세, 이용약관, 개인정보처리방침, 위치 기반 서비스 약관을 표시한다. |
| 경로(Route) | /support/* |
| 인증/권한(Auth/Permission) | guest_or_member |
| 레이아웃(Layout) | apps/site mobile shell + bottom GNB where applicable |
| Primary test-id | site-scr-016 |
...
| API | API-044 GET /api/support/notices - 공지사항 목록<br>API-045 GET /api/support/notices/{contentId} - 공지사항 상세<br>API-046 GET /api/support/policies/{contentType} - 약관/개인정보 정책 조회<br>API-086 POST /api/analytics/events - 분석 이벤트 수집 |
...
| Main Content | 고객지원 공지와 정책 콘텐츠 primary content | 권한 허용 및 조회 성공 시 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | site-scr-016-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-044 | 현재 화면/상태 유지 |
| 주요 액션 실행 | site-scr-016-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-045, API-046, API-086 | 현재 화면/상태 유지 |
| 상태 복구 또는 이동 | site-scr-016-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | 현재 화면/상태 유지 |
...
~~~mermaid
flowchart TD
  Entry[/support/* 진입] --> Auth{권한/세션 확인}
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-016 고객지원 공지와 정책 콘텐츠
...
| --- | --- | --- | --- | --- | --- | --- |
| 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | 권한/데이터 fixture 준비 | 화면 진입 | 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | state/action/API mock | site-scr-016-ac-01 | Playwright candidate |
| 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | 권한/데이터 fixture 준비 | 주요 액션 실행 | 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | state/action/API mock | site-scr-016-ac-02 | Playwright candidate |
| 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | 권한/데이터 fixture 준비 | 권한 필요 액션 실행 | 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | state/action/API mock | site-scr-016-ac-03 | Playwright candidate |
