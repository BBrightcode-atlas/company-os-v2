# 화면정의서(Screen Definition) - SITE-SCR-018 통합 검색
...
| 화면 설명(Screen Description) | 명의, 병원, 커뮤니티 탭별 통합 검색과 결과 없음/더 보기/스크롤 보존을 제공한다. |
| 경로(Route) | /search |
...
| Main Content | 통합 검색 primary content | 권한 허용 및 조회 성공 시 |
...
| 주요 액션 실행 | site-scr-018-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-008, API-009, API-010 | SITE-SCR-005 |
| 상태 복구 또는 이동 | site-scr-018-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | SITE-SCR-005 |
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-018 통합 검색
