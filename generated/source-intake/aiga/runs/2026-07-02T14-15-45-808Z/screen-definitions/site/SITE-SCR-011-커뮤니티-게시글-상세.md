# 화면정의서(Screen Definition) - SITE-SCR-011 커뮤니티 게시글 상세
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 커뮤니티 게시글 상세 |
| 화면 코드(Screen Code) | SITE-SCR-011 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 게시글 본문, 이미지, 공감, 댓글/대댓글, 신고, 수정/삭제 동선을 제공한다. |
| 경로(Route) | /community/posts/:postId |
| 인증/권한(Auth/Permission) | guest_limited_or_member |
...
| Main Content | 커뮤니티 게시글 상세 primary content | 권한 허용 및 조회 성공 시 |
...
| 주요 액션 실행 | site-scr-011-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-008, API-022, API-025 | SITE-SCR-012 |
| 상태 복구 또는 이동 | site-scr-011-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | SITE-SCR-012 |
...
~~~mermaid
flowchart TD
  Entry[/community/posts/:postId 진입] --> Auth{권한/세션 확인}
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-011 커뮤니티 게시글 상세
