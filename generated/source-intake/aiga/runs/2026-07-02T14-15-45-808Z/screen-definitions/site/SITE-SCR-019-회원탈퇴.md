# 화면정의서(Screen Definition) - SITE-SCR-019 회원탈퇴
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 회원탈퇴 |
| 화면 코드(Screen Code) | SITE-SCR-019 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 탈퇴 필수 안내 체크, 탈퇴 처리, 완료 팝업, 24시간 재가입 제한 안내를 제공한다. |
| 경로(Route) | /my/withdrawal |
...
| --- | --- |
| 스키마(Schema) | SCH-001, SCH-013, SCH-022 |
| API | API-002 GET /api/me - 현재 사용자 세션과 등급 조회<br>API-006 DELETE /api/me - 회원 탈퇴<br>API-086 POST /api/analytics/events - 분석 이벤트 수집 |
| 기능 정의서(Feature Definition) | FEA-002 인증, 로그인, 세션, SNS 계정 통합<br>FEA-012 마이페이지, 내 활동, 고객지원, 회원탈퇴<br>FEA-022 개인정보, 접근성, 분석 이벤트 |
...
| Main Content | 회원탈퇴 primary content | 권한 허용 및 조회 성공 시 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | site-scr-019-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-002 | 현재 화면/상태 유지 |
| 주요 액션 실행 | site-scr-019-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-006, API-086 | SITE-SCR-001 |
| 상태 복구 또는 이동 | site-scr-019-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | SITE-SCR-001 |
...
~~~mermaid
flowchart TD
  Entry[/my/withdrawal 진입] --> Auth{권한/세션 확인}
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-019 회원탈퇴
  participant API as API-002
