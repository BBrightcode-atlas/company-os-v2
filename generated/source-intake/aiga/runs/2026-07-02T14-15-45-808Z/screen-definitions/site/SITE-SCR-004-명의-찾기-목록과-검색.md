# 화면정의서(Screen Definition) - SITE-SCR-004 명의 찾기 목록과 검색
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 명의 찾기 목록과 검색 |
| 화면 코드(Screen Code) | SITE-SCR-004 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 명의 찾기 검색, 진료과/질환 필터, 위치 fallback, 의사 목록과 빈 상태를 제공한다. |
| 경로(Route) | /doctors |
...
| --- | --- |
| 스키마(Schema) | SCH-002, SCH-003, SCH-006, SCH-009, SCH-021, SCH-022 |
| API | API-007 GET /api/guest-limits - 비회원 사용 한도 조회<br>API-008 POST /api/guest-limits/events - 비회원 제한 이벤트 차감<br>API-009 GET /api/search - 통합 검색<br>API-010 GET /api/doctors - 명의 찾기 의사 목록<br>API-011 GET /api/doctors/{doctorProfileId} - 의사 프로필 상세<br>API-083 GET /api/me/saved-doctors - 저장한 의료진 목록<br>API-084 POST /api/doctors/{doctorProfileId}/save - 의사 프로필 저장<br>API-085 DELETE /api/doctors/{doctorProfileId}/save - 의사 프로필 저장 취소<br>API-086 POST /api/analytics/events - 분석 이벤트 수집 |
| 기능 정의서(Feature Definition) | FEA-003 비회원 제한, 한도, 로그인 유도<br>FEA-010 의사 프로필, 명의 찾기, 정보 수정 요청<br>FEA-011 통합 검색, 명의 찾기 검색, 입력 보안<br>FEA-013 공통 UI 상태, 네비게이션, 중복 액션 방지<br>FEA-016 홈 운영과 사용자 홈 노출<br>FEA-022 개인정보, 접근성, 분석 이벤트 |
...
| Main Content | 명의 찾기 목록과 검색 primary content | 권한 허용 및 조회 성공 시 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | site-scr-004-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-007 | 현재 화면/상태 유지 |
| 주요 액션 실행 | site-scr-004-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-008, API-009, API-010 | SITE-SCR-005 |
| 상태 복구 또는 이동 | site-scr-004-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | SITE-SCR-005 |
...
~~~mermaid
flowchart TD
  Entry[/doctors 진입] --> Auth{권한/세션 확인}
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-004 명의 찾기 목록과 검색
