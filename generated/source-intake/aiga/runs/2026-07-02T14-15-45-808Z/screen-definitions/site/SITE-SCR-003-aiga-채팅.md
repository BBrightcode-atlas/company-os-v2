# 화면정의서(Screen Definition) - SITE-SCR-003 AIGA 채팅
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | AIGA 채팅 |
| 화면 코드(Screen Code) | SITE-SCR-003 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 외부 AI REST API와 연동해 채팅 메시지를 전송하고 추천 의사/병원 결과로 연결한다. |
| 경로(Route) | /aiga |
...
| --- | --- |
| 스키마(Schema) | SCH-001, SCH-002, SCH-014, SCH-003, SCH-015, SCH-022 |
| API | API-001 GET /api/app-context - 사이트 앱 컨텍스트 조회<br>API-049 POST /api/ai-chat/messages - 외부 AI 채팅 REST 메시지 전송<br>API-086 POST /api/analytics/events - 분석 이벤트 수집 |
| 기능 정의서(Feature Definition) | FEA-010 의사 프로필, 명의 찾기, 정보 수정 요청<br>FEA-013 공통 UI 상태, 네비게이션, 중복 액션 방지<br>FEA-022 개인정보, 접근성, 분석 이벤트<br>FEA-023 외부 AI 채팅 REST API 연동 |
...
| Main Content | AIGA 채팅 primary content | 권한 허용 및 조회 성공 시 |
...
| 주요 액션 실행 | site-scr-003-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-049, API-086 | SITE-SCR-005 |
| 상태 복구 또는 이동 | site-scr-003-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | SITE-SCR-005 |
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-003 AIGA 채팅
