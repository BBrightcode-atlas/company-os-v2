# 화면정의서(Screen Definition) - SITE-SCR-014 내 활동
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 내 활동 |
| 화면 코드(Screen Code) | SITE-SCR-014 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 내 게시글, 댓글, 후기 탭과 최신순 목록, 상세 이동, 빈 상태를 제공한다. |
| 경로(Route) | /my/activity |
| 인증/권한(Auth/Permission) | member |
| 레이아웃(Layout) | apps/site mobile shell + bottom GNB where applicable |
| Primary test-id | site-scr-014 |

## 2. 참조 계약(Referenced Contracts)

| 구분(Type) | 참조(Reference) |
| --- | --- |
| 스키마(Schema) | SCH-002, SCH-006, SCH-007, SCH-008, SCH-012, SCH-003, SCH-009, SCH-010, SCH-019, SCH-014, SCH-013, SCH-021, SCH-022 |
| API | API-022 GET /api/community/posts/{postId} - 커뮤니티 게시글 상세<br>API-034 GET /api/doctors/{doctorProfileId}/reviews - 의사 후기 목록<br>API-037 DELETE /api/reviews/{reviewId} - 의사 후기 삭제<br>API-039 GET /api/comment - 리뷰 댓글 목록<br>API-082 GET /api/me/activity - 내 활동 집계 조회<br>API-086 POST /api/analytics/events - 분석 이벤트 수집 |
| 기능 정의서(Feature Definition) | FEA-004 커뮤니티 피드, 글쓰기, 상세, 임시저장<br>FEA-005 댓글, 대댓글, 공감 인터랙션<br>FEA-006 의사 리뷰와 후기 작성<br>FEA-012 마이페이지, 내 활동, 고객지원, 회원탈퇴<br>FEA-022 개인정보, 접근성, 분석 이벤트 |
...
| Main Content | 내 활동 primary content | 권한 허용 및 조회 성공 시 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | site-scr-014-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-022 | 현재 화면/상태 유지 |
| 주요 액션 실행 | site-scr-014-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-034, API-037, API-039 | SITE-SCR-011 |
| 상태 복구 또는 이동 | site-scr-014-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | SITE-SCR-011 |
...
~~~mermaid
flowchart TD
  Entry[/my/activity 진입] --> Auth{권한/세션 확인}
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-014 내 활동
...
| --- | --- | --- | --- | --- | --- | --- |
| 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | 권한/데이터 fixture 준비 | 화면 진입 | 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | state/action/API mock | site-scr-014-ac-01 | Playwright candidate |
| 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | 권한/데이터 fixture 준비 | 주요 액션 실행 | 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | state/action/API mock | site-scr-014-ac-02 | Playwright candidate |
| 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | 권한/데이터 fixture 준비 | 권한 필요 액션 실행 | 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | state/action/API mock | site-scr-014-ac-03 | Playwright candidate |
