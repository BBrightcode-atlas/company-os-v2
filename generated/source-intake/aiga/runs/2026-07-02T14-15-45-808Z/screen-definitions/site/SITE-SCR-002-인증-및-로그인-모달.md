# 화면정의서(Screen Definition) - SITE-SCR-002 인증 및 로그인 모달
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 인증 및 로그인 모달 |
| 화면 코드(Screen Code) | SITE-SCR-002 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 카카오/네이버 로그인, 최초 가입 약관, 세션 만료, pendingAction 복귀를 처리한다. |
| 경로(Route) | /auth/* modal |
...
| --- | --- |
| 스키마(Schema) | SCH-001, SCH-002, SCH-014, SCH-022 |
| API | API-001 GET /api/app-context - 사이트 앱 컨텍스트 조회<br>API-002 GET /api/me - 현재 사용자 세션과 등급 조회<br>API-003 PATCH /api/me/profile - 내 프로필 수정<br>API-004 POST /api/auth/oauth/{provider}/callback - SNS 로그인 콜백 처리<br>API-005 POST /api/auth/logout - 로그아웃<br>API-086 POST /api/analytics/events - 분석 이벤트 수집 |
| 기능 정의서(Feature Definition) | FEA-001 회원 등급 및 권한 정책<br>FEA-002 인증, 로그인, 세션, SNS 계정 통합<br>FEA-003 비회원 제한, 한도, 로그인 유도<br>FEA-013 공통 UI 상태, 네비게이션, 중복 액션 방지<br>FEA-022 개인정보, 접근성, 분석 이벤트 |
...
| Main Content | 인증 및 로그인 모달 primary content | 권한 허용 및 조회 성공 시 |
...
| 주요 액션 실행 | site-scr-002-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-002, API-003, API-004 | previous_screen |
| 상태 복구 또는 이동 | site-scr-002-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | previous_screen |
...
~~~mermaid
flowchart TD
  Entry[/auth/* modal 진입] --> Auth{권한/세션 확인}
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-002 인증 및 로그인 모달
