# 화면정의서(Screen Definition) - SITE-SCR-012 신고 모달
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 신고 모달 |
| 화면 코드(Screen Code) | SITE-SCR-012 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 게시글, 댓글, 후기, 후기 댓글 신고 사유를 선택하고 접수 완료 상태를 표시한다. |
| 경로(Route) | /reports/new modal |
...
| --- | --- |
| 스키마(Schema) | SCH-012, SCH-016, SCH-018, SCH-009, SCH-013, SCH-022 |
| API | API-032 POST /api/community/reports - 커뮤니티 콘텐츠 신고<br>API-038 POST /api/reviews/{reviewId}/reports - 의사 후기 신고<br>API-086 POST /api/analytics/events - 분석 이벤트 수집 |
...
| Main Content | 신고 모달 primary content | 권한 허용 및 조회 성공 시 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | site-scr-012-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-032 | 현재 화면/상태 유지 |
| 주요 액션 실행 | site-scr-012-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-038, API-086 | previous_screen |
| 상태 복구 또는 이동 | site-scr-012-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | previous_screen |
...
~~~mermaid
flowchart TD
  Entry[/reports/new modal 진입] --> Auth{권한/세션 확인}
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-012 신고 모달
