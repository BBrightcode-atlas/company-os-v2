# 화면정의서(Screen Definition) - SITE-SCR-005 의사 프로필 상세
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 의사 프로필 상세 |
| 화면 코드(Screen Code) | SITE-SCR-005 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 의사 상세, 리뷰, 저장, 정보 수정 요청, 본인 프로필 관리 진입을 제공한다. |
| 경로(Route) | /doctors/:doctorProfileId |
| 인증/권한(Auth/Permission) | guest_or_member_with_limits |
...
| --- | --- |
| 스키마(Schema) | SCH-002, SCH-003, SCH-009, SCH-004, SCH-013, SCH-010, SCH-019, SCH-012, SCH-016, SCH-020, SCH-021, SCH-022 |
| API | API-007 GET /api/guest-limits - 비회원 사용 한도 조회<br>API-008 POST /api/guest-limits/events - 비회원 제한 이벤트 차감<br>API-011 GET /api/doctors/{doctorProfileId} - 의사 프로필 상세<br>API-012 POST /api/doctors/{doctorProfileId}/change-requests - 의사 정보 수정 요청<br>API-034 GET /api/doctors/{doctorProfileId}/reviews - 의사 후기 목록<br>API-038 POST /api/reviews/{reviewId}/reports - 의사 후기 신고<br>API-039 GET /api/comment - 리뷰 댓글 목록<br>API-040 POST /api/comment - 리뷰 댓글 작성<br>API-041 PUT /api/comment/{commentId} - 리뷰 댓글 수정<br>API-042 DELETE /api/comment/{commentId} - 리뷰 댓글 삭제<br>API-081 GET /api/files/{fileId}/signed-url - 파일 signed URL 조회<br>API-084 POST /api/doctors/{doctorProfileId}/save - 의사 프로필 저장<br>API-085 DELETE /api/doctors/{doctorProfileId}/save - 의사 프로필 저장 취소<br>API-086 POST /api/analytics/events - 분석 이벤트 수집 |
| 기능 정의서(Feature Definition) | FEA-001 회원 등급 및 권한 정책<br>FEA-003 비회원 제한, 한도, 로그인 유도<br>FEA-006 의사 리뷰와 후기 작성<br>FEA-007 병원 방문 인증과 증빙 뱃지<br>FEA-009 의사 인증 표시와 실명 전환<br>FEA-010 의사 프로필, 명의 찾기, 정보 수정 요청<br>FEA-012 마이페이지, 내 활동, 고객지원, 회원탈퇴<br>FEA-014 신고, 블라인드, 운영 처리<br>FEA-022 개인정보, 접근성, 분석 이벤트 |
...
| Main Content | 의사 프로필 상세 primary content | 권한 허용 및 조회 성공 시 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | site-scr-005-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-007 | 현재 화면/상태 유지 |
| 주요 액션 실행 | site-scr-005-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-008, API-011, API-012 | SITE-SCR-006 |
| 상태 복구 또는 이동 | site-scr-005-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | SITE-SCR-006 |
...
~~~mermaid
flowchart TD
  Entry[/doctors/:doctorProfileId 진입] --> Auth{권한/세션 확인}
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-005 의사 프로필 상세
