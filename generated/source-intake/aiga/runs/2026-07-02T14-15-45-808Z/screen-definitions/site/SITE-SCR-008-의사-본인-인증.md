# 화면정의서(Screen Definition) - SITE-SCR-008 의사 본인 인증
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 의사 본인 인증 |
| 화면 코드(Screen Code) | SITE-SCR-008 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 본인확인 채널 선택, 면허번호 제출, 재시도 한도, 완료/오류 상태를 처리한다. |
...
| Main Content | 의사 본인 인증 primary content | 권한 허용 및 조회 성공 시 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | site-scr-008-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-013 | 현재 화면/상태 유지 |
| 주요 액션 실행 | site-scr-008-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-014, API-015, API-016 | SITE-SCR-013 |
| 상태 복구 또는 이동 | site-scr-008-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | SITE-SCR-013 |
...
~~~mermaid
flowchart TD
  Entry[/doctor-verification 진입] --> Auth{권한/세션 확인}
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-008 의사 본인 인증
