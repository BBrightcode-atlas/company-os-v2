# 화면정의서(Screen Definition) - SITE-SCR-013 마이페이지 프로필과 더보기
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 마이페이지 프로필과 더보기 |
| 화면 코드(Screen Code) | SITE-SCR-013 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 비회원 로그인 유도, 회원 프로필, 닉네임 수정, 고객지원/약관/탈퇴 진입점을 제공한다. |
| 경로(Route) | /my |
...
| --- | --- |
| 스키마(Schema) | SCH-001, SCH-002, SCH-014, SCH-017, SCH-006, SCH-007, SCH-009, SCH-019, SCH-021, SCH-022 |
| API | API-001 GET /api/app-context - 사이트 앱 컨텍스트 조회<br>API-002 GET /api/me - 현재 사용자 세션과 등급 조회<br>API-003 PATCH /api/me/profile - 내 프로필 수정<br>API-005 POST /api/auth/logout - 로그아웃<br>API-044 GET /api/support/notices - 공지사항 목록<br>API-045 GET /api/support/notices/{contentId} - 공지사항 상세<br>API-046 GET /api/support/policies/{contentType} - 약관/개인정보 정책 조회<br>API-047 POST /api/feedback - 의견 보내기<br>API-082 GET /api/me/activity - 내 활동 집계 조회<br>API-086 POST /api/analytics/events - 분석 이벤트 수집 |
| 기능 정의서(Feature Definition) | FEA-001 회원 등급 및 권한 정책<br>FEA-002 인증, 로그인, 세션, SNS 계정 통합<br>FEA-012 마이페이지, 내 활동, 고객지원, 회원탈퇴<br>FEA-013 공통 UI 상태, 네비게이션, 중복 액션 방지<br>FEA-017 공지사항과 정책 콘텐츠 관리<br>FEA-018 의견 보내기와 운영자 종결<br>FEA-022 개인정보, 접근성, 분석 이벤트 |
...
| Main Content | 마이페이지 프로필과 더보기 primary content | 권한 허용 및 조회 성공 시 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | site-scr-013-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-001 | 현재 화면/상태 유지 |
| 주요 액션 실행 | site-scr-013-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-002, API-003, API-005 | SITE-SCR-014 |
| 상태 복구 또는 이동 | site-scr-013-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | SITE-SCR-014 |
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-013 마이페이지 프로필과 더보기
