# 화면정의서(Screen Definition) - SITE-SCR-001 홈
...
| 화면 설명(Screen Description) | 홈 큐레이션, 검색 진입, 베스트 닥터, 커뮤니티 추천, AIGA 채팅 진입을 제공한다. |
| 경로(Route) | / |
...
| --- | --- |
| 스키마(Schema) | SCH-001, SCH-002, SCH-014, SCH-003, SCH-006, SCH-018, SCH-016, SCH-011, SCH-017, SCH-015, SCH-022 |
| API | API-001 GET /api/app-context - 사이트 앱 컨텍스트 조회<br>API-007 GET /api/guest-limits - 비회원 사용 한도 조회<br>API-008 POST /api/guest-limits/events - 비회원 제한 이벤트 차감<br>API-009 GET /api/search - 통합 검색<br>API-010 GET /api/doctors - 명의 찾기 의사 목록<br>API-020 GET /api/community - 커뮤니티 공간 목록<br>API-021 GET /api/community/posts - 커뮤니티 게시글 목록<br>API-033 GET /api/community/feed/home - 홈 커뮤니티 피드 조회<br>API-043 GET /api/home - 홈 화면 데이터 조회<br>API-048 GET /api/operational-policies/effective - 적용 중 운영 정책 조회<br>API-049 POST /api/ai-chat/messages - 외부 AI 채팅 REST 메시지 전송<br>API-086 POST /api/analytics/events - 분석 이벤트 수집 |
...
| Main Content | 홈 primary content | 권한 허용 및 조회 성공 시 |
...
| 주요 액션 실행 | site-scr-001-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-007, API-008, API-009 | SITE-SCR-018 |
| 상태 복구 또는 이동 | site-scr-001-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | SITE-SCR-018 |
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-001 홈
