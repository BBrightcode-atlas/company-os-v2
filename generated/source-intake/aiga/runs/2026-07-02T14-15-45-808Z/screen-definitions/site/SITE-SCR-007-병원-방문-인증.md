# 화면정의서(Screen Definition) - SITE-SCR-007 병원 방문 인증
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 병원 방문 인증 |
| 화면 코드(Screen Code) | SITE-SCR-007 |
| 대상 surface(Target Surface) | site |
| 화면 설명(Screen Description) | 영수증/처방전 이미지를 업로드하고 외부 OCR 판독 상태와 수동 검토 fallback을 안내한다. |
| 경로(Route) | /medical-visit-verifications/new modal |
...
| --- | --- |
| 스키마(Schema) | SCH-009, SCH-010, SCH-016, SCH-020, SCH-024, SCH-025, SCH-013, SCH-015 |
| API | API-018 POST /api/medical-visit-verifications - 병원 방문 인증 제출<br>API-019 GET /api/medical-visit-verifications/{verificationId} - 병원 방문 인증 상태 조회<br>API-079 POST /api/files/upload-url - 파일 업로드 URL 생성<br>API-080 POST /api/files/confirm - 파일 업로드 확정<br>API-081 GET /api/files/{fileId}/signed-url - 파일 signed URL 조회<br>API-094 GET /api/admin/integration-call-logs - 외부 연동 호출 로그 조회<br>API-095 GET /api/admin/integrations/status - 외부 연동 상태 조회 |
...
| Main Content | 병원 방문 인증 primary content | 권한 허용 및 조회 성공 시 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | site-scr-007-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-018 | 현재 화면/상태 유지 |
| 주요 액션 실행 | site-scr-007-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-019, API-079, API-080 | SITE-SCR-006 |
| 상태 복구 또는 이동 | site-scr-007-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | SITE-SCR-006 |
...
~~~mermaid
flowchart TD
  Entry[/medical-visit-verifications/new modal 진입] --> Auth{권한/세션 확인}
...
sequenceDiagram
  actor User as 사용자
  participant Screen as SITE-SCR-007 병원 방문 인증
