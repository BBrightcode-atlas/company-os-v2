# 화면정의서(Screen Definition) - ADMIN-SCR-011 운영 정책 설정
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 운영 정책 설정 |
| 화면 코드(Screen Code) | ADMIN-SCR-011 |
| 대상 surface(Target Surface) | admin |
| 화면 설명(Screen Description) | 수정/삭제 가능 기간, 비회원 한도, 리뷰/댓글 정책 등 운영 정책을 조회하고 저장한다. |
| 경로(Route) | /admin/operational-policies |
| 인증/권한(Auth/Permission) | admin_operator |
| 레이아웃(Layout) | apps/admin shell/table/form layout |
| Primary test-id | admin-scr-011 |

## 2. 참조 계약(Referenced Contracts)

| 구분(Type) | 참조(Reference) |
| --- | --- |
| 스키마(Schema) | SCH-014, SCH-013, SCH-001, SCH-023 |
| API | API-048 GET /api/operational-policies/effective - 적용 중 운영 정책 조회<br>API-076 GET /api/admin/operational-policies - 운영 정책 목록<br>API-077 PUT /api/admin/operational-policies/{policyKey} - 운영 정책 저장<br>API-078 GET /api/admin/audit-logs - 감사 로그 조회<br>API-090 GET /api/admin/auth/session - 어드민 세션 조회 |
| 기능 정의서(Feature Definition) | FEA-005 댓글, 대댓글, 공감 인터랙션<br>FEA-006 의사 리뷰와 후기 작성<br>FEA-007 병원 방문 인증과 증빙 뱃지<br>FEA-013 공통 UI 상태, 네비게이션, 중복 액션 방지<br>FEA-021 운영 정책 설정<br>FEA-022 개인정보, 접근성, 분석 이벤트 |
...
| Main Content | 운영 정책 설정 primary content | 권한 허용 및 조회 성공 시 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | admin-scr-011-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-048 | 현재 화면/상태 유지 |
| 주요 액션 실행 | admin-scr-011-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-076, API-077, API-078 | 현재 화면/상태 유지 |
| 상태 복구 또는 이동 | admin-scr-011-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | 현재 화면/상태 유지 |
...
~~~mermaid
flowchart TD
  Entry[/admin/operational-policies 진입] --> Auth{권한/세션 확인}
...
sequenceDiagram
  actor User as 사용자
  participant Screen as ADMIN-SCR-011 운영 정책 설정
  participant API as API-048
...
| --- | --- | --- | --- | --- | --- | --- |
| 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | 권한/데이터 fixture 준비 | 화면 진입 | 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | state/action/API mock | admin-scr-011-ac-01 | Playwright candidate |
| 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | 권한/데이터 fixture 준비 | 주요 액션 실행 | 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | state/action/API mock | admin-scr-011-ac-02 | Playwright candidate |
| 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | 권한/데이터 fixture 준비 | 권한 필요 액션 실행 | 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | state/action/API mock | admin-scr-011-ac-03 | Playwright candidate |
