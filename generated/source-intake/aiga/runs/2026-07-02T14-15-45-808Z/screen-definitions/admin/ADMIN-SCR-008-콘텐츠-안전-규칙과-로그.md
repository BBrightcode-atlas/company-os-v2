# 화면정의서(Screen Definition) - ADMIN-SCR-008 콘텐츠 안전 규칙과 로그
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 콘텐츠 안전 규칙과 로그 |
| 화면 코드(Screen Code) | ADMIN-SCR-008 |
| 대상 surface(Target Surface) | admin |
| 화면 설명(Screen Description) | 금칙어/이미지 안전 규칙, 자동 차단 로그, 외부 moderation 호출 상태를 관리한다. |
| 경로(Route) | /admin/content-safety |
| 인증/권한(Auth/Permission) | admin_operator |
| 레이아웃(Layout) | apps/admin shell/table/form layout |
| Primary test-id | admin-scr-008 |

## 2. 참조 계약(Referenced Contracts)

| 구분(Type) | 참조(Reference) |
| --- | --- |
| 스키마(Schema) | SCH-016, SCH-013, SCH-001, SCH-023, SCH-024, SCH-025, SCH-020, SCH-015 |
| API | API-064 GET /api/admin/content-safety/rules - 콘텐츠 안전 규칙 목록<br>API-065 POST /api/admin/content-safety/rules - 콘텐츠 안전 규칙 생성<br>API-066 PATCH /api/admin/content-safety/rules/{ruleId} - 콘텐츠 안전 규칙 수정<br>API-078 GET /api/admin/audit-logs - 감사 로그 조회<br>API-090 GET /api/admin/auth/session - 어드민 세션 조회<br>API-093 GET /api/admin/content-safety/logs - 콘텐츠 안전 로그 조회<br>API-094 GET /api/admin/integration-call-logs - 외부 연동 호출 로그 조회<br>API-095 GET /api/admin/integrations/status - 외부 연동 상태 조회 |
| 기능 정의서(Feature Definition) | FEA-015 콘텐츠 안전 자동 차단<br>FEA-020 어드민 사용자, 의사, 콘텐츠 관리<br>FEA-022 개인정보, 접근성, 분석 이벤트 |
...
| Main Content | 콘텐츠 안전 규칙과 로그 primary content | 권한 허용 및 조회 성공 시 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | admin-scr-008-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-064 | 현재 화면/상태 유지 |
| 주요 액션 실행 | admin-scr-008-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-065, API-066, API-078 | 현재 화면/상태 유지 |
| 상태 복구 또는 이동 | admin-scr-008-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | 현재 화면/상태 유지 |
...
~~~mermaid
flowchart TD
  Entry[/admin/content-safety 진입] --> Auth{권한/세션 확인}
...
sequenceDiagram
  actor User as 사용자
  participant Screen as ADMIN-SCR-008 콘텐츠 안전 규칙과 로그
  participant API as API-064
...
| --- | --- | --- | --- | --- | --- | --- |
| 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | 권한/데이터 fixture 준비 | 화면 진입 | 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | state/action/API mock | admin-scr-008-ac-01 | Playwright candidate |
| 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | 권한/데이터 fixture 준비 | 주요 액션 실행 | 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | state/action/API mock | admin-scr-008-ac-02 | Playwright candidate |
| 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | 권한/데이터 fixture 준비 | 권한 필요 액션 실행 | 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | state/action/API mock | admin-scr-008-ac-03 | Playwright candidate |
