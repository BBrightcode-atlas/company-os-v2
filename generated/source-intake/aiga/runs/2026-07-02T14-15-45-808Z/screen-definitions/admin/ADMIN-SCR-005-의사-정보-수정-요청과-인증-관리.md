# 화면정의서(Screen Definition) - ADMIN-SCR-005 의사 정보 수정 요청과 인증 관리
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 의사 정보 수정 요청과 인증 관리 |
| 화면 코드(Screen Code) | ADMIN-SCR-005 |
| 대상 surface(Target Surface) | admin |
| 화면 설명(Screen Description) | 의사 정보 수정 요청, 의사 인증 요청 승인/반려, 면허 검증 이력을 처리한다. |
| 경로(Route) | /admin/doctor-requests |
| 인증/권한(Auth/Permission) | admin_operator |
| 레이아웃(Layout) | apps/admin shell/table/form layout |
| Primary test-id | admin-scr-005 |

## 2. 참조 계약(Referenced Contracts)

| 구분(Type) | 참조(Reference) |
| --- | --- |
| 스키마(Schema) | SCH-003, SCH-004, SCH-013, SCH-001, SCH-005, SCH-023 |
| API | API-054 GET /api/admin/doctor-profile-change-requests - 의사 정보 수정 요청 목록<br>API-055 PATCH /api/admin/doctor-profile-change-requests/{requestId} - 의사 정보 수정 요청 처리<br>API-056 GET /api/admin/doctor-verifications - 의사 인증 요청 목록<br>API-057 PATCH /api/admin/doctor-verifications/{requestId} - 의사 인증 요청 승인/반려<br>API-078 GET /api/admin/audit-logs - 감사 로그 조회<br>API-090 GET /api/admin/auth/session - 어드민 세션 조회 |
| 기능 정의서(Feature Definition) | FEA-008 의사 인증 플로우와 면허번호 검증<br>FEA-009 의사 인증 표시와 실명 전환<br>FEA-010 의사 프로필, 명의 찾기, 정보 수정 요청<br>FEA-020 어드민 사용자, 의사, 콘텐츠 관리<br>FEA-022 개인정보, 접근성, 분석 이벤트 |
| Product Builder Base 참고 | apps/admin auth/community/table/form patterns |

## 3. 화면 구성(Screen Composition)

| 영역(Area) | 컴포넌트(Component) | 표시 조건(Display Rule) |
| --- | --- | --- |
| Shell/Header | Admin shell, route guard, toolbar | surface 공통 레이아웃 |
| Main Content | 의사 정보 수정 요청과 인증 관리 primary content | 권한 허용 및 조회 성공 시 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | admin-scr-005-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-054 | 현재 화면/상태 유지 |
| 주요 액션 실행 | admin-scr-005-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-055, API-056, API-057 | ADMIN-SCR-004 |
| 상태 복구 또는 이동 | admin-scr-005-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | ADMIN-SCR-004 |
...
~~~mermaid
flowchart TD
  Entry[/admin/doctor-requests 진입] --> Auth{권한/세션 확인}
...
sequenceDiagram
  actor User as 사용자
  participant Screen as ADMIN-SCR-005 의사 정보 수정 요청과 인증 관리
  participant API as API-054
...
| --- | --- | --- | --- | --- | --- | --- |
| 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | 권한/데이터 fixture 준비 | 화면 진입 | 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | state/action/API mock | admin-scr-005-ac-01 | Playwright candidate |
| 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | 권한/데이터 fixture 준비 | 주요 액션 실행 | 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | state/action/API mock | admin-scr-005-ac-02 | Playwright candidate |
| 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | 권한/데이터 fixture 준비 | 권한 필요 액션 실행 | 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | state/action/API mock | admin-scr-005-ac-03 | Playwright candidate |
