# 화면정의서(Screen Definition) - ADMIN-SCR-001 어드민 로그인 및 2FA
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 어드민 로그인 및 2FA |
| 화면 코드(Screen Code) | ADMIN-SCR-001 |
| 대상 surface(Target Surface) | admin |
| 화면 설명(Screen Description) | 운영자 이메일/비밀번호 로그인, Google OTP 검증, 세션 조회/로그아웃, 2FA 설정/해제를 처리한다. |
| 경로(Route) | /admin/login |
| 인증/권한(Auth/Permission) | admin_operator |
| 레이아웃(Layout) | apps/admin shell/table/form layout |
| Primary test-id | admin-scr-001 |

## 2. 참조 계약(Referenced Contracts)

| 구분(Type) | 참조(Reference) |
| --- | --- |
| 스키마(Schema) | SCH-001, SCH-013, SCH-023 |
| API | API-087 POST /api/admin/auth/login - 어드민 이메일/비밀번호 로그인<br>API-088 POST /api/admin/auth/2fa/verify - 어드민 Google OTP 검증<br>API-089 POST /api/admin/auth/logout - 어드민 로그아웃<br>API-090 GET /api/admin/auth/session - 어드민 세션 조회<br>API-091 POST /api/admin/auth/2fa/setup - 어드민 2FA 설정<br>API-092 DELETE /api/admin/auth/2fa - 어드민 2FA 비활성화 |
| 기능 정의서(Feature Definition) | FEA-019 어드민 공통 운영, 권한, 감사 로그<br>FEA-022 개인정보, 접근성, 분석 이벤트 |
...
| Main Content | 어드민 로그인 및 2FA primary content | 권한 허용 및 조회 성공 시 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | admin-scr-001-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-087 | 현재 화면/상태 유지 |
| 주요 액션 실행 | admin-scr-001-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-088, API-089, API-090 | ADMIN-SCR-002 |
| 상태 복구 또는 이동 | admin-scr-001-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | ADMIN-SCR-002 |
...
~~~mermaid
flowchart TD
  Entry[/admin/login 진입] --> Auth{권한/세션 확인}
...
sequenceDiagram
  actor User as 사용자
  participant Screen as ADMIN-SCR-001 어드민 로그인 및 2FA
  participant API as API-087
...
| --- | --- | --- | --- | --- | --- | --- |
| 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | 권한/데이터 fixture 준비 | 화면 진입 | 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | state/action/API mock | admin-scr-001-ac-01 | Playwright candidate |
| 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | 권한/데이터 fixture 준비 | 주요 액션 실행 | 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | state/action/API mock | admin-scr-001-ac-02 | Playwright candidate |
| 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | 권한/데이터 fixture 준비 | 권한 필요 액션 실행 | 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | state/action/API mock | admin-scr-001-ac-03 | Playwright candidate |
