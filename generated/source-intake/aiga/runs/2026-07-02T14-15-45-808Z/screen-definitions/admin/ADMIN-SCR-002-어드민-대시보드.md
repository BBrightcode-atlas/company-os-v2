# 화면정의서(Screen Definition) - ADMIN-SCR-002 어드민 대시보드
...
| --- | --- |
| 프로젝트(Project) | Aiga |
| 화면명(Screen Name) | 어드민 대시보드 |
| 화면 코드(Screen Code) | ADMIN-SCR-002 |
| 대상 surface(Target Surface) | admin |
| 화면 설명(Screen Description) | 사용자, 의사, 인증, 신고, 공지, 운영 지표 요약과 주요 큐 진입점을 제공한다. |
| 경로(Route) | /admin |
| 인증/권한(Auth/Permission) | admin_operator |
| 레이아웃(Layout) | apps/admin shell/table/form layout |
| Primary test-id | admin-scr-002 |

## 2. 참조 계약(Referenced Contracts)

| 구분(Type) | 참조(Reference) |
| --- | --- |
| 스키마(Schema) | SCH-001, SCH-003, SCH-005, SCH-009, SCH-012, SCH-013, SCH-017, SCH-023 |
| API | API-050 GET /api/admin/dashboard - 어드민 대시보드 조회<br>API-078 GET /api/admin/audit-logs - 감사 로그 조회<br>API-090 GET /api/admin/auth/session - 어드민 세션 조회 |
| 기능 정의서(Feature Definition) | FEA-019 어드민 공통 운영, 권한, 감사 로그<br>FEA-020 어드민 사용자, 의사, 콘텐츠 관리<br>FEA-022 개인정보, 접근성, 분석 이벤트 |
...
| Main Content | 어드민 대시보드 primary content | 권한 허용 및 조회 성공 시 |
...
| --- | --- | --- | --- | --- | --- |
| 화면 진입 및 초기 조회 | admin-scr-002-act-01 | route 진입 또는 모달 열기 | 권한과 초기 데이터를 확인하고 상태를 표시한다. | API-050 | 현재 화면/상태 유지 |
| 주요 액션 실행 | admin-scr-002-act-02 | 주요 CTA, 목록 항목, 저장, 제출, 검색, 탭 선택 | 선행 API 계약 또는 클라이언트 상태 전환으로 결과를 반영한다. | API-078, API-090 | ADMIN-SCR-005 |
| 상태 복구 또는 이동 | admin-scr-002-act-03 | 재시도, 뒤로가기, 닫기, 상세 이동 | 오류/권한/empty 상태에서 복구하거나 대상 화면으로 이동한다. | N/A | ADMIN-SCR-005 |
...
sequenceDiagram
  actor User as 사용자
  participant Screen as ADMIN-SCR-002 어드민 대시보드
  participant API as API-050
...
| --- | --- | --- | --- | --- | --- | --- |
| 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | 권한/데이터 fixture 준비 | 화면 진입 | 초기 진입 시 default/empty/loading/error/permission 상태가 구분된다. | state/action/API mock | admin-scr-002-ac-01 | Playwright candidate |
| 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | 권한/데이터 fixture 준비 | 주요 액션 실행 | 주요 액션 실행 시 중복 제출이 방지되고 성공/실패 결과가 화면에 반영된다. | state/action/API mock | admin-scr-002-ac-02 | Playwright candidate |
| 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | 권한/데이터 fixture 준비 | 권한 필요 액션 실행 | 권한이 부족한 사용자는 로그인 유도 또는 운영자 권한 차단 상태를 본다. | state/action/API mock | admin-scr-002-ac-03 | Playwright candidate |
